...
describe('canAccessPartner', () => {
    test('PDM (admin) can access any partner', () => {
        const admin = createMockUser('PDM', 'admin@example.com');
...
describe('filterPartnersByRole', () => {
    ...
    test('PDM (admin) sees all partners', () => {
        const admin = createMockUser('PDM', 'admin@example.com');
...
describe('canViewGate', () => {
    ...
    test('PDM can view all gates', () => {
        ...
        expect(canViewGate('PDM', 'gate-2')).toBe(true);
        expect(canViewGate('PDM', 'gate-3')).toBe(true);
        expect(canViewGate('PDM', 'post-launch')).toBe(true);
    });
});
...
describe('groupPartnersByGate', () => {
    ...
    test('groups partners by gate for PDM (all gates)', () => {
        const pdm = createMockUser('PDM', 'pam@example.com');
        const grouped = groupPartnersByGate(partners, pdm);

        expect(grouped['pre-contract']).toHaveLength(2);
        expect(grouped['gate-1']).toHaveLength(1);
        expect(grouped['gate-2']).toHaveLength(1); // PDM sees all gates
    });
});

describe('canEditPartner', () => {
    test('PDM (admin) can edit any partner', () => {
        const admin = createMockUser('PDM', 'admin@example.com');
        const partner = createMockPartner();
...
describe('canSubmitQuestionnaire', () => {
    test('PDM (admin) can submit any questionnaire', () => {
        const admin = createMockUser('PDM', 'admin@example.com');
        const partner = createMockPartner();

        expect(canSubmitQuestionnaire(admin, partner, 'gate-1')).toBe(true);
        expect(canSubmitQuestionnaire(admin, partner, 'gate-2')).toBe(true);
    });

    test('PAM can submit questionnaire for their partner', () => {
        const pam = createMockUser('PAM', 'pam@example.com');
        const partner = createMockPartner({ pamOwner: 'pam@example.com' });

        expect(canSubmitQuestionnaire(pam, partner, 'gate-1')).toBe(true);
    });

    test('PAM cannot submit questionnaire for partner they do not own', () => {
        const pam = createMockUser('PAM', 'other@example.com');
        const partner = createMockPartner({ pamOwner: 'pam@example.com' });

        expect(canSubmitQuestionnaire(pam, partner, 'gate-1')).toBe(false);
    });
});
...
describe('getAssignedPartners', () => {
    const partners: PartnerRecord[] = [
        ...
        createMockPartner({ id: 'p2', pamOwner: 'user@example.com' }),
        createMockPartner({ id: 'p3', pamOwner: 'other@example.com' }),
    ];

    test('returns partners assigned to user as PAM owner', () => {
...
describe('getRoleDashboardMessage', () => {
    test('returns correct message for each role', () => {
        expect(getRoleDashboardMessage('PDM')).toContain('all partners');
        expect(getRoleDashboardMessage('PAM')).toContain('assigned partners');
    });
});
